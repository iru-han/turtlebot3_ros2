...
bool Turtlebot3Sensor::getFlameDigitalData(void)
{
    // 사용자 코드 반영: HIGH일 때 FIRE!
    return digitalRead(flame_pin_);
}

float Turtlebot3Sensor::getFlameAnalogData(void)
{
    return (float)analogRead(flame_analog_pin_);
}
...
bool Turtlebot3Sensor::getGasDigitalData(void)
{
    // 사용자 코드 반영: LOW일 때 SMOKE!
    return digitalRead(gas_digital_pin_);
}

float Turtlebot3Sensor::getGasAnalogData(void)
{
